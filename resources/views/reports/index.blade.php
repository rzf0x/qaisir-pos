<x-app-layout>
    <!-- Header -->
    <div class="mb-6">
        <h2 class="text-xl font-bold text-slate-800">Laporan Usaha</h2>
    ...
    <div class="flex space-x-2 mb-6 overflow-x-auto pb-2">
        <a href="{{ route('reports.index', ['period' => 'today']) }}"
            class="flex-shrink-0 px-4 py-2 rounded-xl text-sm font-medium transition-all
                  {{ $period === 'today' ? 'bg-slate-800 text-white shadow-lg' : 'bg-white text-gray-600 card-shadow hover:bg-gray-50' }}">
            Hari Ini
        </a>
        <a href="{{ route('reports.index', ['period' => 'week']) }}"
            class="flex-shrink-0 px-4 py-2 rounded-xl text-sm font-medium transition-all
                  {{ $period === 'week' ? 'bg-slate-800 text-white shadow-lg' : 'bg-white text-gray-600 card-shadow hover:bg-gray-50' }}">
            7 Hari Terakhir
        </a>
        <a href="{{ route('reports.index', ['period' => 'month']) }}"
            class="flex-shrink-0 px-4 py-2 rounded-xl text-sm font-medium transition-all
                  {{ $period === 'month' ? 'bg-slate-800 text-white shadow-lg' : 'bg-white text-gray-600 card-shadow hover:bg-gray-50' }}">
            Bulan Ini
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 mb-6">
        <!-- Total Income -->
        <div class="p-6 rounded-2xl bg-gradient-to-br from-slate-700 to-slate-900 text-white card-shadow">
        ...
        <div class="grid grid-cols-2 gap-4">
            <!-- Transaction Count -->
            <div class="p-5 rounded-2xl bg-white card-shadow border border-slate-100">
            ...
            <div class="p-5 rounded-2xl bg-white card-shadow border border-slate-100">
                ...
                <p class="text-xl font-bold text-slate-800">{{ $summary['formatted_cash'] }}</p>
            </div>
        </div>

        <!-- QR Income -->
        <div class="p-4 rounded-xl bg-white card-shadow border border-slate-100 flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-slate-800">{{ $summary['formatted_qr'] }}</p>
            </div>
        </div>
    </div>
    ...
    @if ($period !== 'today' && count($dailyBreakdown) > 0)
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-slate-800 mb-4">Ringkasan Harian</h3>
            <div class="space-y-2">
                @foreach ($dailyBreakdown as $day)
                    <div
                        class="p-3 rounded-xl bg-white card-shadow border border-slate-100 flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-slate-800">{{ $day['date'] }}</p>
        ...
        @if ($transactions->count() > 0)
            <div class="space-y-2">
                @foreach ($transactions as $transaction)
                    <div class="p-3 rounded-xl bg-white card-shadow border border-slate-100">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <div class="flex items-center space-x-2 mb-1">
                                    <p class="text-sm font-medium text-slate-800">{{ $transaction->service_name }}</p>
                                    <span
